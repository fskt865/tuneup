...
**Option 8, Triage, is the whole-machine pass**: it runs the core report plus
every module read-only, ranks what it found (CRITICAL / WARNING / UNKNOWN /
INFO — UNKNOWN means a check could not run, never that it passed), then walks
the automated fixes **one typed YES at a time**. Nothing applies in bulk. If
any disk finding is critical, heavy-I/O fixes (DISM/SFC, Windows Update) are
gated behind typing `DATA IS SAFE` — recover first, repair after. A scripted
`-Action Triage` run scans and prints the fix plan as commands, but never
applies anything: there is no one at the keyboard to say yes.

Non-interactive:

```powershell
.\Invoke-TuneUp.ps1 -Action Report
.\Invoke-TuneUp.ps1 -Action Triage                           # scan + plan, applies nothing
...
The first three are genuinely open source rather than free-for-personal-use —
which matters on a paid bench, where OCCT and HWiNFO's personal licences do not
...
| Everything (voidtools) | Freeware, **closed source** | Instant filename search read from the MFT |
...
#### Everything is the exception, on both counts

voidtools publishes no source and the terms are whatever the EULA in the
download says — read it before assuming a paid bench is covered. It is here
anyway because nothing open source searches a volume this fast, and *find the
customer's data before touching anything* is most of a Level 1 job. Windows
Search is indexed, partial and slow, and on a machine that is already unwell
the index is usually the first thing that stopped working.

It is also the only tool here that manufactures customer data as a side effect.
Everything builds an index of every filename on the machine it runs on, and in
portable mode it writes that index beside its own exe — **on the stick**. A
complete file listing of a customer's disk, leaving on removable media. So:

```
tools\Everything\SEARCH.cmd     launch with this
tools\Everything\Everything.exe do not launch this
```

Three layers, because one is not enough for the rule this breaks:

1. **`SEARCH.cmd` passes `-nodb`.** The index is built in RAM from the MFT and
   dies with the process — nothing written anywhere. It self-elevates first:
   unelevated, Everything offers to install its service instead, which is a
   permanent change to a machine you were handed to repair. Costs a few seconds
   of re-indexing per launch, which is the entire price of this arrangement.
2. **`Everything.ini` sets `db_location` to `C:\ProgramData\GSTuneUp`,** so even
   a bare double-click of the exe puts the index on the customer's own machine
   rather than on the stick — and menu option 7 (Purge) deletes that directory
   with everything else the toolkit leaves behind. The ini is written
   **read-only**, because Everything saves the current search text back into it
   on exit and a search box on a real job has a customer's name in it. Search
   history, run history, update checks and both built-in servers are off.
3. **`Deploy-ToUsb.ps1` looks for an index on the stick** and reports it in red
   with the `Remove-Item` line. Layer 2 is a fallback and fallbacks fail quietly;
   a deploy is the last moment before the stick goes back out.

Both files are generated by `Install-Tools.ps1 -CopyToStick`, not committed —
`tools\` is gitignored, so the config is code in the repo rather than an
artifact someone has to remember to copy.
...
D:\  exFAT, ~14.4 GB          Ventoy data partition
     ...
     tuneup\                        the toolkit; not an ISO, so invisible to the menu
         tools\                     smartmontools, LibreHardwareMonitor, CrystalDiskInfo
             Everything\SEARCH.cmd  filename search, launched here and nowhere else
